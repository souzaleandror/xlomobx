...
import 'package:xlomobx/repositories/favorite_repository.dart';
...
abstract class _FavoriteStore with Store {
  ...
  Future<void> _getFavoriteList() async {
    try {
      favoriteList.clear();
      final favorites =
          await FavoriteRepository().getFavorites(userManagerStore.userModel);
    ...
    try {
      if (favoriteList.any((a) => a.id == ad.id)) {
        ...
        await FavoriteRepository()
            .delete(ad: ad, user: userManagerStore.userModel);
      } else {
        favoriteList.add(ad);
        await FavoriteRepository()
            .save(ad: ad, user: userManagerStore.userModel);
      }
    } catch (e) {
      print(e);
    }
